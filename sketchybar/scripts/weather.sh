...
# get KEY, CITY, NAME from the config file
source "$SCRIPT_DIR/weather.env.sh"
...
weather=""
for i in {1..4}; do
  weather="$(curl -sf --retry 3 "$API?key=$KEY&q=$CITY")" # shuck: ignore=C156
  if [[ -z "$weather" || "$(echo "$weather" | jq -r ".current")" == "null" ]]; then
    echo "[WARNING] [$(date -Iseconds)] Weather API request failed (attempt $i)" >&2
    if ((i == 4)); then
      echo "[ERROR] [$(date -Iseconds)] Weather API request failed 4 times" >&2
      exit 0
    fi
  else
    break
...
sketchybar --set "$NAME" drawing=on icon="$icon" label="${temp%%.*}°C" \
  ...
  --set weather.lastupdated label="Last Updated: $(date +%T)"
